package io.github.maturapoj.tokenflow.presentation.theme
...
@Composable
fun TokenFlowTheme(darkTheme: Boolean = isSystemInDarkTheme(), content: @Composable () -> Unit) {
    MaterialTheme(colorScheme = if (darkTheme) DarkColors else LightColors, content = content)
}

/** Navy top bar: primary in light mode; in dark mode primary is light blue, so use the navy container. */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun navyTopAppBarColors(): TopAppBarColors {
    val c = MaterialTheme.colorScheme
    val (container, content) = if (isSystemInDarkTheme()) c.primaryContainer to c.onPrimaryContainer else c.primary to c.onPrimary
