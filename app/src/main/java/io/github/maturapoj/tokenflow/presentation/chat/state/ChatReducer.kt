...
fun reduce(state: ChatState, result: ChatResult): ChatState = when (result) {
    ...
    is ChatResult.SettingsChanged -> state.copy(
        isConfigured = result.isConfigured,
        selectedModelId = state.selectedModelId.ifEmpty { result.lastModelId },
        modelGroups = if (result.isConfigured) state.modelGroups else emptyList(),
    )
}
